...
import { maintenanceActivitiesCache } from "../utils/seed";
import { useAccessData } from "../composables/useAccessData";

export const useMaintenanceActivityStore = defineStore("maintenanceactivity", {
  state: () => ({
    items: [],
    loading: false,
    loaded: false,
  ...
  actions: {
    async init(force = false) {
      ...
      try {
        ...
        const result = getScopedData(maintenanceActivitiesCache);
        this.hydrate(result);
      } catch (err) {
