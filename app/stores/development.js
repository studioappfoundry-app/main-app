...
import { developmentActivitiesCache } from "../utils/seed";
import { useAccessData } from "../composables/useAccessData";

export const useDevelopmentActivityStore = defineStore("developmentactivity", {
  state: () => ({
    items: [],
    loading: false,
    loaded: false,
  ...
  actions: {
    async init(force = false) {
      ...
      try {
        ...
        const result = getScopedData(developmentActivitiesCache);
        this.hydrate(result);
      } catch (err) {
