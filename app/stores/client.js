...
import { clientBusinessesCache } from "../utils/seed";

export const useClientBusinessStore = defineStore("clientbusiness", {
  state: () => ({
    items: [],
    loading: false,
    loaded: false,
  ...
  actions: {
    async init(force = false) {
      ...
      try {
        ...
        const result = getScopedData(clientBusinessesCache);
        this.hydrate(result);
      } catch (err) {
